...
\begin{algorithmic}[1]
  \Function {IdentifyFreezeDeadlocks}{position}
    \If { wall on left or right }
      \State $horizontallyBlocked\gets true$
    \ElsIf { simple deadlock on on left and right }
      \State $horizontallyBlocked\gets true$
    \ElsIf { box on left or right }
      \State $horizontallyBlocked\gets IdentifyFreezeDeadlocks(left) or IdentifyFreezeDeadlocks(right)$
    \EndIf

    \If { wall on up or down }
      \State $verticallyBlocked\gets true$
    \ElsIf { simple deadlock on on up and down }
      \State $verticallyBlocked\gets true$
    \ElsIf { box on up or down }
      \State $verticallyBlocked\gets IdentifyFreezeDeadlocks(up) or IdentifyFreezeDeadlocks(down)$
    \EndIf
    \State \Return horizontallyBlocked and verticallyBlocked
  \EndFunction
  \end{algorithmic}
\end{algorithm}
  ...
  \begin{algorithmic}[3]
    \Function {RunEpisode}{state, Q}
      ...
      \While {not Terminal?(state)}
        ...
        \State $Q(state, action)\gets \textnormal{$Q(state, action) + LEARNING_RATE 
                * [Reward(state, action) + DISCOUNT_FACTOR 
                * MaximizeQ(resultState) - $Q(state, action)}$
      \EndWhile
      \Return Q
    \EndFunction
  \end{algorithmic}
